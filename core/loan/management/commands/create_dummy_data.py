...
from authentik.models import DefaultUser
from banking.models import Transaction, BankAccount
from loan.models import Loan, LoanRepayment, LoanRequest, CreditScore
...
class Command(BaseCommand):
    ...
    @staticmethod
    def generate_ten_digit_number(prefix):
        prefix = str(prefix)
        suffix = str(random.randint(000000, 999999))
        account_number = int(prefix + suffix)
        return account_number

    def handle(self, *args, **kwargs):
        # Create Lenders and Borrowers
        lenders, created = Group.objects.get_or_create(name="lenders")
        borrowers, created = Group.objects.get_or_create(name="borrowers")
